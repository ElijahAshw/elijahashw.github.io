...
/** TODO:
   * Eliminate hard coded numbers
   * Write comment para about game; big picture, what did I create
   * Consider pointing out obsticals and innovations
   * Consider making a game constructor
   * Submit!
**/ //
    ...
    "home": {
        run: function (keys, secondsElapsed) {
            ...
            for (var i = 0; i < snowPositions.length; i++) {
                ...
                var y = snowPositions[i].y - height / 80;
                ellipse(x, y, 10, 10);
                snowPositions[i].y = (snowPositions[i].y + 1) % (height * 1.025);
            }
        ...
        buttons: [
            {x: 35, y: 285, w: 100, message: "Tutorial", textSize: 24, keys: "t",
                onclick: function () { Screen.switchTo("info"); }},
    ...
    "play": {
        ...
        buttons: [
            ...
            {x: 120, y: 10, w: 50, h: 50, color: color(74, 74, 74, 100),
                hoverColor: color(125, 125, 125, 200), strokeWeight: 1,
                selectedColor: color(201, 201, 201, 100), keys: "3", 
                message: function(x, y, w, h, canvas) {
                    x += w / 2;
                    y += h / 3;
            ...
            {x: 280, y: 10, w: 50, h: 50, color: color(74, 74, 74, 100),
                hoverColor: color(125, 125, 125, 200), strokeWeight: 1,
                selectedColor: color(201, 201, 201, 100), textSize: 13, keys: "6",
                message: "Straight\nsnow", onclick: "select1", selected: true},
            
            // Random snow
            {x: 330, y: 10, w: 50, h: 50, color: color(74, 74, 74, 100),
                hoverColor: color(125, 125, 125, 200), strokeWeight: 1,
                selectedColor: color(201, 201, 201, 100), textSize: 13, keys: "7",
    ...
    "end": {
        run: function (keys) {
            ...
            for (var i = 0; i < message.length; i++) {
                text(message[i], width / 2, 130 + i * 29);
            }
            
        },
        buttons: [
            {x: width / 2 - 240 / 2, y: 300, w: 240, h: 68, message: "Restart game", 
            textSize: 33, color: color(153, 252, 96), keys: "r",
            onclick: function () {
                gameState.level = 0;
    ...
    "select": {
        ...
        buttons: [
            {x: width * 0.125, y: height * 0.8, w: width * 0.25, h: height * 0.155, 
                r: width * 0.0275, message: "Home", textSize: width * 0.0625, keys: "h",
                onclick: function () { Screen.switchTo("home"); }},
            {x: width * 0.5, y: height * 0.8, w: width * 0.375, h: height * 0.155, 
                r: width * 0.0275, textSize: width * 0.055,
                message: "Resume\nplay (c)", keys: "c",
                onclick: function () {
...
var Level = (function() {
    ...
    Level.prototype.createWeather = function(weather, straightSnow) {
        ...
        if (weather === "light snow" || weather === "snow") {
            ...
            */
            if (frameCount % speed === 0) {
                ...
                // Check for collisions to avoid creating snow on top of other objects
                // Snow can be created on top of empty or grate squares
                if (!this.touches(newSnowX, 0, "not empty grate")) {
                    var collides = false;
                    // Check for actor collisions
                    outerLoop: // Label the outer loop so it can be broken easily
        ...
        // Sun
        if (weather === "sun" || weather === "small sun") {
    ...
    Level.prototype.touchesActors = function(actor1, types) {
        ...
        for (var t = 0; t < types.length; t++) {
            // The actors are sorted into their types, so we only need to check those
            var type = types[t];
            for (var j = 0; j < this.actors[type].length; j++) {
                ...
                if (actor1 !== actor2 && !actor2.remove &&
                    ...
                    return actor2; // We found a collision
                }
            }
        }
