  ...
  collide(state, player) {
    ...
    if(this.passNum > 1) {
      state = state.replaceActor(this, 
          new Portal(this.pos, this.passNum - 1, this.angle));
      filtered = state.actors.filter(a => a != player);
    } else if(this.passNum > 0) {
      filtered = state.actors.filter(
        a => a != this && a != player);
    }
    let status = state.status;
...
class Player {
  ...
  static create(pos, char) {
    ...
    if(char === "n") {
      return new Player(pos, new Vec(null, 0), 
        jumpSpeed, playerXSpeed, gravity, 1, {char});
    } else if(char === "r") {
      return new Player(pos, new Vec(null, 0), 
        jumpSpeed, -playerXSpeed, gravity, 1, {char});
    } else if(char === "y") {
      ...
      return new Player(pos, new Vec(null, 0), 
        0, playerXSpeed, gravity, 1, {char});
    } else if(char === "u") {
      return new Player(pos, new Vec(null, 0), 
        -jumpSpeed, playerXSpeed, -gravity, 1, {char});
    } else if(char === "s") {
      return new Player(pos, new Vec(null, 0), 
        jumpSpeed, 0, gravity, 1, {char});
    } else if(char === "w") {
      return new Player(pos, new Vec(null, 0), 
        jumpSpeed, playerXSpeed, gravity, -1, {char});
    }
  ...
  fromNewPos(pos, xSpeed, ySpeed, dirX, dirY, jumpSpeed = this.jumpSpeed, gravity = this.gravity) {
    ...
    return new Player(pos, speed, jumpSpeed, this.xSpeed, 
        gravity, this.toggle, this.sig, dir, this.pos);
  }
