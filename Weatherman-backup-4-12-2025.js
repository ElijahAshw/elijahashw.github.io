...
var allLevelsStored = [
    /* Level 1 */ [
        ...
        "---g--------------^------g",
        "--$g>-------------gs-gg######g",
        "gggggg###ggggggggggggggggggggggggggggggggg"
    ],
    /* Level 2 */ [
        "",
        "---------",
...
var gameScreensStored = {
    ...
    "info": {
        run: function () {
            ...
            text("Use the arrow keys to move the player\naround. Wasd and/or spacebar also work.\nUse the selectors at the top of the screen\nto change the weather, and use this to\nyour advantage.\nAvoid lava and enemies. However,\nyou can eliminate enemies by jumping\n on top of them.", width / 2, 100);
        },
        buttons: [
    ...
    "play": {
        run: function (keys, secondsElapsed, selected) {
            ...
            // This is done before running the level to give you one more frame
            ...
            } else if (allLevels[gameState.level].state === "lose") {
                // Restart the level
                Level.loadLevel();
            }
...
var Sun = (function() {
    ...
    Sun.prototype.check = function(level) {
        ...
        var touchLavaY = level.touches(this.x, this.y, "lava");
        var touchPortalY = level.touches(this.x, this.y, "portal");
        var touchY = touchDirtY || touchLavaY || touchPortalY;
        if (touchY) {
            this.remove = true;
        }
    };
...
var Snow = (function() {
    ...
    Snow.prototype.collide = function(actor, level) {
        ...
        if (actor.type === "enemy" || actor.type === "snow") {
...
var Enemy = (function() {
    ...
    Enemy.prototype.update = function(secondsElapsed, keys, level) {
        ...
        var touchX = touchDirtX || touchPortalX;
        var touchDirtXY = level.touches(newX, this.y + 1, "dirt");
        if (touchX) {
            if (xMotion < 0) {
                this.x = touchX.x + blockSize;
            } else if (xMotion > 0) {
    ...
    Enemy.prototype.collide = function(actor) {
        ...
        if (actor.type === "snow" || actor.type === "enemy") {
            if (collideDirection === "side") {
                if (this.direction === LEFT) {
                    this.x = actor.x + actor.s;
                } else if (this.direction === RIGHT) {
                    this.x = actor.x - this.s;
                }
                this.turn();
            }
        }
    ...
    function Level(plan) {
        ...
        for (var y = 0; y < plan.length; y++) {
            ...
            for (var x = 0; x < plan[y].length; x++) {
                ...
                // A string means a static block that will be passed to the Static constuctor
                if (typeof type === "string" || type === undefined) {
                    ...
                    this.actors.push(new type(xPos, yPos, blockSize, plan[y][x]));
                }
            }
    ...
    Level.prototype.makeItSnow = function(weather, straightSnow) {
        if (weather === weatherTypes[3] || weather === weatherTypes[4]) {
            ...
            if (frameCount % speed === 0) {
                ...
                if (!this.touches(newSnowX, 0, "dirt") &&
                !this.touches(newSnowX, 0, "lava") && 
                !this.touches(newSnowX, 0, "portal")) {
    ...
    Level.prototype.update = function(keys, secondsElapsed) {
        ...
        for (var i = 0; i < this.actors.length; i++) {
            ...
            for (var j = i + 1; j < this.actors.length; j++) {
                ...
                if (actor2.type === "player") { continue; } // Yes, skip the players
