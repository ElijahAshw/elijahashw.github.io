 ...
 * and for some performance suggestions.
 * However, I did *not* copy paste *any* AI code into this project.
 * 
 * Fortunately, I think I have maintained bugs < lines of code in this project!
 * 
 * Thanks!
 * 
 ...
 * Low priortity:
 *    Reuse vertices
 *    Merge long walls (and move seg generation into separate step)
 *    Move colorblind mode button and add symmetric debug mode button
...
var currentLightType = "internal"; // Todo: change to "raycast" when finished
...
var cheated = false;
var transition = {
    /* Transition states go "done" > "out" > "in" */
    state: "done",
...
var player = {
    ...
    firstX: 0, firstY: 0, isStuck: false, currentDir: RIGHT,
    ...
    if (err && err.infiniteLoopNodeType) {
        ...
        if (!clean) {
            if (err) {
                ...
                stackTxt = stackTxt.replace(stackCleanRegExp, "($1...)");
                println(stackTxt);
            }
    ...
    Button.prototype.keyPressed = function(keys) {
        if (this.keys && this.onClick) {
            for (var k = 0; k < this.keys.length; k++) {
                var key = this.keys[k].toLowerCase();
...
function loadLevel(levelMap, levelSegments, lightColor) {
    ...
    levelSegments.push(
        Segment.new(-margin, -margin, lvlPxW + margin, -margin, 0),
        Segment.new(lvlPxW + margin, -margin, lvlPxW + margin, lvlPxH + margin, 0),
        Segment.new(lvlPxW + margin, lvlPxH + margin, -margin, lvlPxH + margin, 0),
        Segment.new(-margin, lvlPxH + margin, -margin, -margin, 0)
    );
...
function drawJewels() {
    ...
    for (var i = 0; i < jewels.x.length; i++) {
        var jewelBounce = Math.sin((jewelTimer + i) * 0.1) * 0.1;
        var jewelPulse = Math.cos((jewelTimer + i) * 0.05) * 0.09;
...
function drawPlayer(x, y, w, h) {
    ...
    fill(0, 0, 0);
    ellipse(x + w*0.3*s + offset, y + h*0.3*s, w*0.15*s, h*0.15*s);
    ellipse(x + w*0.7*s + offset, y + h*0.3*s, w*0.15*s, h*0.15*s);
}
...
function drawStuck(a) {
    ...
    textSize(width*0.13);
    text("Change colors\nto free yourself.", width/2, height*0.7);
}
...
buttonsByScene = {
    ...
    "play": Object.assign({
        ...
        var btnW = width*0.084, gap = btnW / 7;
...
draw = function() {
    ...
    try {
        ...
        if (isTransitionState("done")) {
            ...
            // Still run as exit for play scene death animation
